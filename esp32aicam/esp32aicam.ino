...
// as unset) - this is the recommended path. Only replace these with real
// values if you specifically want credentials baked into the compiled
// binary; if you do, keep that copy out of version control and out of
// anywhere it might get shared, since anyone with the .ino can read them
// straight out of the source.
#define DEFAULT_WIFI_SSID     "YourWiFiSSID"
#define DEFAULT_WIFI_PASSWORD "YourWiFiPassword"
#define DEFAULT_OPENAI_API_KEY "sk-YourOpenAIKeyHere"
...
unsigned long daylightOpenedAt = 0;  // 0 = no pending post-dawn settling window
...
// Detections::add() only logs confirmed birds - most triggers (stage1
// "no bird" rejections from wind/leaves/exposure flicker) leave no record
// anywhere. Keep a small independent ring of every trigger's timestamp so
// cooldown behavior can be checked after the fact via /api/status, instead
// of needing a live serial connection at the exact moment something looks
// wrong.
time_t recentTriggers[10] = {0};
...
void runPipeline(String instantPath, String settledPath) {
  ...
  if (chosen.length() == 0) {
    ...
    // Keep the most recent rejection viewable instead of deleting it
    // outright - with stage1_pass sitting at 0 previously, there was no
    // way to tell whether the motion gate is firing on non-bird movement
    // (wind, glare, foliage) or whether stage1 is missing real birds.
    // Served automatically: /birds/*.jpg is already routed to SD.
    const String lastMiss = "/birds/_last_miss.jpg";
    String keep = settledPath.length() ? settledPath : instantPath;
    String drop = (keep == settledPath) ? instantPath : settledPath;
    if (drop.length() && drop != keep) SD_MMC.remove(drop);
    if (keep.length()) {
      if (SD_MMC.exists(lastMiss)) SD_MMC.remove(lastMiss);
      if (!SD_MMC.rename(keep, lastMiss)) SD_MMC.remove(keep);
    }
...
void birdLoop() {
  ...
  if (now - lastLightRead >= LIGHT_INTERVAL_MS || lastLightRead == 0) {
    ...
    if (daylight && !wasDaylight) {
      // The gate below skips MotionDetector::check() entirely while dark,
      // so its reference frame (prevGrid) goes stale for the whole night -
      // last thing it saw was dusk. The instant daylight flips true, the
      // very next check would otherwise compare this morning's bright
      // frame against last night's dark one: near-total scene difference,
      // guaranteed false trigger on lighting alone, every single dawn.
      // Reset so the first post-dawn frame only primes the grid, and hold
      // off a bit longer while auto-exposure is still actively settling
      // into the new light level (same reasoning as the boot warm-up
      // below, just triggered by a lighting transition instead of reset).
      MotionDetector::reset();
      daylightOpenedAt = now;
      Serial.println("Daylight gate opened - motion baseline reset, settling");
    }
  ...
  // Same reasoning, triggered by the dawn transition rather than boot:
  // give auto-exposure time to settle into the new light level before
  // trusting frame-to-frame comparisons again.
  if (daylightOpenedAt && now - daylightOpenedAt < 15000) return;
  ...
  if (motion && gatesOpen) {
    ...
    recentTriggers[recentTriggerCount % 10] = time(nullptr);
    recentTriggerCount++;
...
void handleApiStatus() {
  ...
  doc["cooldown_remaining_s"] = (cooldownUntil > nowMs) ? (uint32_t)((cooldownUntil - nowMs) / 1000) : 0;
  JsonArray rt = doc["recent_triggers"].to<JsonArray>();
  int n = min(recentTriggerCount, 10);
  for (int i = 0; i < n; i++) {
    int idx = (recentTriggerCount - n + i) % 10;
    rt.add((uint32_t)recentTriggers[idx]);
  }
  struct tm t;
  bool timeSynced = getLocalTime(&t, 0);
...
// Accepts a real JSON number, or a string that cleanly parses as one -
// but NOT blank/malformed strings. Needed because .as<int>() on an empty
// string silently returns 0, which constrain() then clamps to the floor
// of its valid range (cooldown_s -> 5, motion_pct -> 1, etc.) - actively
// applying a bad value instead of safely leaving the field untouched.
// A config page that resends every field on save, including ones left
// blank, would otherwise wreck tuning on every single save.
static bool numericField(JsonVariant v, int& out) {
  if (v.isNull()) return false;
  if (v.is<int>() || v.is<float>()) { out = v.as<int>(); return true; }
  if (v.is<String>()) {
    ...
    if (end == s.c_str() || *end != '\0') return false;  // not a clean integer
    out = (int)val;
    return true;
  }
  return false;
...
void handleApiConfigPost() {
  ...
  // numericField() rather than is<int>(): accepts real numbers or clean
  // numeric strings, but ignores blank/malformed ones instead of letting
  // .as<int>() silently coerce them to 0 and get clamped to the range
  // floor - see numericField()'s comment above.
  int v;
  if (numericField(doc["pub_threshold"], v)) BirdConfig::pubThreshold = constrain(v, 50, 100);
  if (numericField(doc["rev_threshold"], v)) BirdConfig::revThreshold = constrain(v, 0, 79);
  if (numericField(doc["cooldown_s"], v))    BirdConfig::cooldownS = constrain(v, 5, 600);
  if (numericField(doc["daily_cap"], v))     BirdConfig::dailyCap = constrain(v, 2, 500);
  if (!doc["daylight_only"].isNull()) {
    if (doc["daylight_only"].is<bool>()) {
      BirdConfig::daylightOnly = doc["daylight_only"].as<bool>();
    } else {
      String s = doc["daylight_only"].as<String>();
      s.trim();
      if (s.length() > 0) {
        s.toLowerCase();
        BirdConfig::daylightOnly = (s == "true" || s == "1" || s == "on" || s == "yes");
      }
    }
  }
  if (numericField(doc["lux_threshold"], v)) BirdConfig::luxThreshold = constrain(v, 1, 5000);
  if (numericField(doc["motion_pct"], v))    BirdConfig::motionPct = constrain(v, 1, 80);
...
void handleStream() {
  ...
  // While this loop runs, the synchronous WebServer can't service ANY
  // other request - including /api/config saves and /api/upload - since
  // it only handles one connection at a time. Confirmed in the field: a
  // curl upload hung indefinitely with a live-view tab open elsewhere on
  // the network. Shortened from 90s so an open live-view tab can only
  // block the rest of the API for a few seconds at a time instead of a
  // minute and a half; the frontend already has a manual "restart
  // preview" button for exactly this kind of reconnect.
  unsigned long streamEnd = millis() + 20000UL;
...
void setup() {
  ...
  // Light sensor AFTER the camera, not before: esp32-camera's SCCB driver
  // and LightSensor's Wire.begin() both default to GPIO8/9 on this board
  // (SIOD/SIOC), so whichever inits second wins the pins. setupCamera()
  // only touches SCCB once, during esp_camera_init()+set_vflip/set_hmirror
  // above, and nothing later in loop() touches it again - so bringing
  // Wire up here lets it own the bus uncontested for the rest of runtime.
  // (Originally this ran before setupCamera(), matching the official
  // DFR1154 examples - which broke the light sensor the moment the camera
  // silently reclaimed the shared pins: reads still "succeeded" but always
  // returned 0, since Wire's transactions failed at the hardware level.)
  LightSensor::begin();
  ...
  // configTzTime() only kicks off SNTP asynchronously - it never confirms
  // success. rollDayIfNeeded() in bird_config.h guards on
  // "if (!getLocalTime(&t, 0)) return;" - so if sync never lands (outbound
  // UDP/123 blocked, DNS failure, etc.) the daily counters (API calls,
  // triggers, etc.) silently never roll over at midnight, with no error
  // anywhere. Give it a bounded window here so that failure is visible in
  // serial + /api/status instead of invisible.
  {
    struct tm t;
    bool synced = false;
    for (int i = 0; i < 20 && !synced; i++) {  // up to ~10s; never blocks boot indefinitely
      synced = getLocalTime(&t, 0);
      if (!synced) delay(500);
    }
    if (synced) {
      char buf[20];
      ...
      Serial.println("WARNING: NTP time did not sync within 10s - daily counters "
                      "(API calls/triggers/etc) will NOT roll over at midnight until "
                      "this device gets a valid time sync. Check that outbound UDP/123 "
                      "(NTP) isn't blocked by your router/firewall.");
      noteError("NTP sync failed - daily counters won't reset");
    }
  }
  ...
  server.on("/api/reset-counters", HTTP_POST, []() {
    BirdConfig::forceResetCounters();
