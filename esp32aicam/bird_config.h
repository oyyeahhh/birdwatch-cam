...
public:
  ...
  // Manual escape hatch for POST /api/reset-counters. rollDayIfNeeded()
  // depends entirely on NTP having synced at least once; if it never does
  // (network blocks outbound NTP, etc.) there'd otherwise be no way to
  // unstick a device pinned at its daily cap short of a reflash. If time
  // happens to be available, use it to set counterDay properly so the
  // normal midnight rollover still takes over cleanly once sync recovers;
  // otherwise just clear the counters and leave counterDay as-is.
  static void forceResetCounters() {
    struct tm t;
    if (getLocalTime(&t, 0)) counterDay = t.tm_yday;
    triggersToday = stage1PassToday = idsToday = publishedToday = apiCallsToday = 0;
    saveCounters();
    Serial.println("BirdConfig: counters force-reset via /api/reset-counters");
  }
